...
const MerchandiseController = async (message, lineId, currentStatus) => {
    const actions = {
        initialize: async () => {
        ...
        register_price: async () => {
            const price = Number(message);
            if (isNaN(price))
                return { type: 'text', text: '価格は数字で入力してください。' };
        ...
        register_stock: async () => {
            const stock = Number(message);
            if (isNaN(stock))
                return { type: 'text', text: '在庫数は数字で入力してください。' };
            await DB.update(merchandises)
                .set({ stock })
                .where(eq(merchandises.lineId, lineId))
                .execute();
...
const createMerchandiseFlexMessage = async (lineId) => {
