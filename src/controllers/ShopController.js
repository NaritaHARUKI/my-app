...
const ShopController = async (message, lineId, currentStatus) => {
    const actions = {
        initialize: async () => {
        ...
        search_station: async () => {
            ...
            if (findStation.length === 0)
                return { type: 'text', text: '最寄駅の情報が見つかりませんでした。もう一度入力してください。' };
            await DB.update(status)
...
`
        ...
        register_station: async () => {
            const _validate = (message) => {
                let errs = [];
                let ok = [];
                const submittedStations = message.split('\n').map(s => s.trim()).filter(s => s !== '');
                const uniqueStations = [...new Set(submittedStations)];
                uniqueStations.forEach((uniqueStation) => {
                    const found = STATION_DATA.some((station) => station.station_name === uniqueStation || station.id === Number(uniqueStation));
                    if (found) {
            ...
            const _getStationName = (stationId) => {
...
お店の最寄駅を以下で登録しました。
-----------------------
駅名：${result.data.map(_getStationName).join(',')}
-----------------------
お店の登録が完了しました。
お店の情報は、「お店の情報を確認する」で確認できます。
        ...
        show: async () => {
            const shop = await DB
                ...
                .execute();
            if (shop.length === 0)
                return { type: 'text', text: '登録されているお店はまだありません。' };
...
-----------------------
    ...
    return await actions[currentStatus]();
};
